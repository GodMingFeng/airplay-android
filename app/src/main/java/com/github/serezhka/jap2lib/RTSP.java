...
class RTSP {
    ...
    private byte[] eiv;
    private byte[] shk; // Session key for ChaCha20-Poly1305 (AP2 mode)
        ...
        if (rtspSetup.containsKey("streams")) {
            ...
            switch (type) {
                ...
                case 103:
                    ...
                    // Log all stream keys for debugging
                    log.info("Audio stream keys: " + stream.keySet());
                    ...
                    // Parse shk (ChaCha20-Poly1305 session key)
                    if (stream.containsKey("shk")) {
                        Object shkObj = stream.get("shk");
                        if (shkObj instanceof byte[]) {
                            shk = (byte[]) shkObj;
                        } else if (shkObj instanceof com.dd.plist.NSObject) {
                            shk = (byte[]) ((com.dd.plist.NSObject) shkObj).toJavaObject();
                        }
                        if (shk != null) {
                            log.info("Audio shk: " + Utils.bytesToHex(shk));
                        }
                    } else {
                        log.warn("No shk in audio stream plist!");
                    }
